<template>
    <table class="table table-striped table-sm table-hover table-responsive">
        <thead>
        <tr>
            <th :colspan="Object.keys(columns).length + 1">Records: {{ items.length }} <span
                    v-if="itemsFiltered.length != items.length">({{ itemsFiltered.length}} displayed)</span>
            </th>
        </tr>
        <tr>
            ...
            <th v-for="(colName, colKey) in columns" @click.prevent="sortColumn(colKey)">{{ colName }} <span
                    class="pull-right"><i class="fa fa-sort-asc"
                                          v-if="mySorting.column == colKey && mySorting.order == 'asc'"></i><i
                    class="fa fa-sort-desc" v-if="mySorting.column == colKey && mySorting.order == 'desc'"></i></span>
            </th>
        </tr>
        </thead>
        <tbody>
